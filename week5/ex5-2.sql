...
INNER JOIN users ON tickets.train=11 AND tickets.user=users.id
...
INNER JOIN users ON users.name=tickets.user
GROUP BY users.id
ORDER BY total_distance DESC
LIMIT 6;

SELECT trains.id, types.name AS 'type', 'source'.name AS src_stn, destination.name AS dst_stn,
 ...
 SELECT types.name AS `type`, src.name AS src_stn, dst.name AS dst_stn, trains.departure,
 trains.arrival, Round(types.fare_rate*trains.distance/1000,-2) as fare_rate
 FROM trains INNER JOIN types ON types.id=trains.type
